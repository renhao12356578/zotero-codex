...
export async function callHost(file, name, args) {
  ...
  let response;
  try {
    response = await fetch(url, { method: 'POST', redirect: 'error', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${config.token}` }, body: JSON.stringify({ name, arguments: args }), signal: AbortSignal.timeout(25000) });
  } catch (error) {
