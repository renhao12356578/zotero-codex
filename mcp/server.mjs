...
export async function callHost(file, name, args) {
  ...
  let response;
  try {
    response = await fetch(url, { method: 'POST', redirect: 'error', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${config.token}` }, body: JSON.stringify({ name, arguments: args, client:{version:'0.8.1',nodePath:process.execPath,serverPath:fileURLToPath(import.meta.url)} }), signal: AbortSignal.timeout(25000) });
  } catch (error) {
...
export function createServer(call, native = null) {
  ...
  const server = new Server({ name: 'zotero-codex-mcp', version: '0.8.1' }, {
    capabilities: { tools: {} },
  ...
  server.setRequestHandler(CallToolRequestSchema, async request => {
    try {
      ...
      if (request.params.name === 'zotero_status') {
        ...
        return resultContent({ version: '0.8.1', ready: Boolean(nativeAPI.connected && plugin.connected), nativeAPI, plugin });
      }
...
if (process.argv[1] && import.meta.url === (await import('node:url')).pathToFileURL(await realpath(process.argv[1])).href) {
