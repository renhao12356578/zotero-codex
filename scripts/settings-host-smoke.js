...
(async () => {
  const result = {version:'0.6.0', stage:'preferences', checks:[]};
  ...
  try {
    ...
    await waitFor(()=>win.document.getElementById('zc-version')?.textContent.includes('0.6.0'));
    ...
    await Zotero.HTTP.request('POST',config.url,{headers:{'Zotero-Allowed-Request':'true','Content-Type':'application/json',Authorization:'Bearer '+config.token},body:JSON.stringify({name:'zotero_status',arguments:{},client:{version:'0.6.0',nodePath:'node',serverPath:PathUtils.join(base,'fixture-server.mjs')}})});
    await IOUtils.writeUTF8(PathUtils.join(base,'fixture-server.mjs'),'// Synthetic service path for configuration copy test');
    ui.refresh();
    check('authenticated-client-runtime-detected', settings.state().lastRequestAt && el('server-path').value.endsWith('fixture-server.mjs') && el('server-version').textContent==='0.6.0');
