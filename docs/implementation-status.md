# 当前实现：0.6.1，Better Notes 风格设置页

2026-09-23。在「设置 → Zotero MCP」查看桥接状态、原生 API 开关、Better Notes、最近客户端请求及服务版本。页面结构采用 Better Notes 的原生 groupbox 分区和按钮样式，刷新状态显示在刷新按钮右侧。自动文字/区域捕获开关立即生效并持久保存，关闭会取消对应的自动快照，手动添加仍可用。支持清除快照、复制通用 MCP 配置、定位连接文件、脱敏诊断和版本入口。

Node 服务通过已鉴权的本机请求提供版本及可执行文件/服务脚本路径，便于复制配置；不会自动改写客户端配置。连接检查使用 Zotero 官方本地请求标记，并保留插件令牌鉴权，检查本身不更新客户端访问时间。

验证：40 项自动化测试、14 项真实 Zotero 设置页检查、4 项真实 MCP stdio 检查通过；新版页面样式在隔离 Zotero 800 × 600 窗口渲染检查通过。原生窗口 800 × 600 排版无横向溢出，上下两部分已渲染检查。报告见 `settings-host-result.json` 和 `settings-stdio-result.json`。测试使用隔离文库，通过程序触发真实 DOM 控件和 Reader 事件。

安装包：`dist/zotero-codex-0.6.1.xpi`。更新 XPI 并重新加载 Node MCP 服务后生效。
